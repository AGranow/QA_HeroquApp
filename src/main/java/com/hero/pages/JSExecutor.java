...
public class JSExecutor extends BasePage {
    ...
    public JSExecutor enterDataWithJSE(String name, String password) {
        if (name != null && password != null) {
            ...
            js.executeScript("document.getElementById('password').style.border ='3px solid red';"); // выделяет нужное поле красным
        }
        return this;
    }
    ...
    public JSExecutor clickOnLoginButtonWithJSE() {
        js.executeScript("document.querySelector('.radius').click();"); // используется только cssSelector , xPath не возможно
